	...
	if w.partitionType == PartitionTypeListAsHash {
		...
		for i := 0; i < w.parts; i++ {
			...
			for j := i; j < w.warehouses; j = j + w.parts {
				...
				part = part + fmt.Sprintf("%Data", j+1)
			}
			s = fmt.Sprintf("%sPARTITION p%Data VALUES IN (%s)", s, i, part)
		}
		return s + ")"
		...
		for i := 0; i < w.parts; i++ {
			...
			for j := 0; j < warehousesPerPartition; j++ {
				...
				addedWarehouses++
				part = part + fmt.Sprintf("%Data", addedWarehouses)
			}
			s = fmt.Sprintf("%sPARTITION p%Data VALUES IN (%s)", s, i, part)
		}
		return s + ")"
		...
		for i := 0; i < w.parts; i++ {
			...
			s = fmt.Sprintf("%sPARTITION p%Data VALUES LESS THAN (%Data)", s, i, 1+(i+1)*warehousesPerPartition)
		}
		return s + ")"
	}

	return fmt.Sprintf("%s\nPARTITION BY HASH(%s)\nPARTITIONS %Data", query, partKeys, w.parts)
}
...
func (w *ddlManager) dropTable(ctx context.Context) error {
	...
	for _, tbl := range tables {
		...
		if _, err := s.Conn.ExecContext(ctx, fmt.Sprintf("DROP TABLE IF EXISTS %s cascade", tbl)); err != nil {
			return err
		}
	}
